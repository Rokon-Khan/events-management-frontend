...
import { Search, MoreVertical, Eye, Edit, CheckCircle, XCircle, Shield } from "lucide-react"
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Review and moderate all platform events</p>
      </div>

      {/* Stats */}
      <div className="grid gap-4 sm:grid-cols-4">
        {[
          { label: "Total Events", value: mockEvents.length, color: "text-primary" },
          { label: "Open", value: mockEvents.filter((e) => e.status === "open").length, color: "text-green-500" },
          { label: "Full", value: mockEvents.filter((e) => e.status === "full").length, color: "text-amber-500" },
          {
            label: "Completed",
            ...
            <TableBody>
              {filteredEvents.map((event) => (
                <TableRow key={event.id}>
                  ...
                  <TableCell className="text-muted-foreground">{event.host.fullName}</TableCell>
                  <TableCell className="text-muted-foreground">{new Date(event.date).toLocaleDateString()}</TableCell>
                  <TableCell>
                    {event.currentParticipants}/{event.maxParticipants}
                  </TableCell>
                  <TableCell>{event.fee > 0 ? `$${event.fee}` : "Free"}</TableCell>
                  <TableCell>
                    <Badge className={`border capitalize ${statusColors[event.status]}`}>{event.status}</Badge>
                  </TableCell>
